...
# DEM
def ensemble_input_diversity(input_tensor, idx, bound=1.5, n=10):                             
...
def de_mi_fgsm(feat1, x, model, num_iter, eps, alpha, fp=None, momentum=1, init=False, use_gpu=True):
    if use_gpu:
        feat1 = feat1.cuda()
        x = x.cuda()
        model = model.cuda()
    ...
    with torch.enable_grad():
        for i in range(num_iter):
            ...
            advs = [ensemble_input_diversity(adv, i, bound=args.bound, n=args.n) for i in range(args.n)]
    ...
    with open(logfile, 'a') as fp:
        ...
        for idx in range(1, len(img_list)):
            ...
            try:
                ...
                adv_x = de_mi_fgsm(feat1=feat1, x=target_img, model=model, fp=fp, **kwargs)
...
if __name__ == '__main__':
    ...
    settings = f"iter{args.num_iter}_eps{args.max_epsilon}_bound{args.bound}_n{args.n}"
