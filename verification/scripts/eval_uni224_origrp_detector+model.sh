...
do
    ...
    nohup rlaunch --cpu 2 --memory 16394 -- python -m attack.eval_uni224_origrp_detector+model \
    --i_net panorama.i_epoch_202622.th_0.8308.neupeak \
    --pattern $path \
    --name "${array[6]}/${array[7]}/${array[8]}" > eval_uni224_origrp_detector+model.out &
    # python -m attack.eval_uni_origrp_detector+model \
    # --i_net panorama.i_epoch_202622.th_0.8308.neupeak \
    # --pattern $path \
