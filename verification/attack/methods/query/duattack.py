...
class UniscoreAttack():
    ...
    def train(self, img_list, y, resume=False):
        ...
        # todo: init stripe pattern Q and momentum 
        assert h == w
        Q = np.eye(h)[None, None, :, :]
        grads = np.zeros([1, c, h, w]).astype(np.float32)
        ...
        for i_iter in range(int(n_queries - 1), self.n_queries):
            ...
            # todo: noise
            shifts = np.random.randint(0, h, 3)
            shifted_Q = np.concatenate([np.roll(Q, s, axis=-1) for s in shifts], axis=1)
            new_deltas = self.eps * shifted_Q + 0.9 * grads

            x_new_pos = x_best_curr + new_deltas
            x_new_pos = np.minimum(np.maximum(x_new_pos, img_batch - self.eps), img_batch + self.eps)
            x_new_pos = np.clip(x_new_pos, 0., 255.)
            margin_pos, loss_pos = self.margin_and_loss(x_new_pos, y)
            
            x_new_neg = x_best_curr - new_deltas
            x_new_neg = np.minimum(np.maximum(x_new_neg, img_batch - self.eps), img_batch + self.eps)
            x_new_neg = np.clip(x_new_neg, 0., 255.)
            margin_neg, loss_neg = self.margin_and_loss(x_new_neg, y)

            if loss_pos.sum() < loss_neg.sum():
                grads = grads + self.eps * shifted_Q
                loss_min = loss_pos
                margin_min = margin_pos
                x_best = x_new_pos
            else:
                grads = grads - self.eps * shifted_Q
                loss_min = loss_neg
                margin_min = margin_neg
                x_best = x_new_neg


            if loss_min.sum() < loss_min_curr.sum():
                logdata = "better >>> nquery:{}, loss_new:{}, loss_cur:{}, suc_new:{}, suc_cur:{}\n".format(
                    n_queries, loss_min.sum().item(), loss_min_curr.sum().item(), (margin_min < 0.0).sum().item(), (margin_min_curr < 0.0).sum().item())

                noise = x_best[0].astype(np.float32) - img_batch[0].astype(np.float32)
                noise = noise[None, :]
                print(noise.min(), noise.max())
                np.save("{}/nq{}@suc{}.npy".format(noise_saving, n_queries, (margin_min < 0.0).sum().item()), noise)
