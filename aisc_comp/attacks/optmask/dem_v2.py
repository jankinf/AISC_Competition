...
def optmask_dem_v2(images, 
    ...
    for i in range(steps):
        ...
        origin_logits = 0
        x_d = [
            ensemble_input_diversity_v2(advs, i, bound, n) for i in range(n)
        ]
        advs = torch.cat(x_d, dim=0)

        for model in models:   
            origin_logits += model(advs)
        origin_logits /= len(models) # shape (n * batch, 512)
        origin_logits = origin_logits.reshape(n, *compare_logits.shape).mean(dim=0)
