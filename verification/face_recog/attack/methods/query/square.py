...
class SquareAttack():
    ...
    def attack_single_run(self, anchor, target_img, fp=None):
        ...
        x = target_img.copy()
        x_min = x.astype(np.float64) - self.eps
        x_max = x.astype(np.float64) + self.eps
        x_best = np.clip(x + self.eps * self.random_choice([h, w, c]), 0., 255.)
        # x_best = x.copy()
        target_feat = self.detector.get_feat(x_best.astype(np.uint8))[0]
        score_min = self.get_score(anchor, target_feat)
        n_queries = 1
        if self.verbose:
            logdata = '{} - # queries={:.1f} - score={:.3f}\n'. \
                format(0, n_queries, score_min)
            print(logdata, end='')
            if fp:
                fp.write(logdata)
                fp.flush()
        ...
        for i_iter in range(self.n_queries):
            ...
            x_new = np.minimum(np.maximum(x_new, x_min), x_max)
            x_new = np.clip(x_new, 0., 255.)

            target_feat = self.detector.get_feat(x_new.astype(np.uint8))[0]
            ...
            if self.verbose:
                logdata = '{} - # queries={:.1f} - score={:.3f}\n'. \
                    format(i_iter + 1, n_queries, score_min)
                print(logdata, end='')
                if fp:
                    fp.write(logdata)
                    fp.flush()
    ...
    def perturb(self, x1, x2, fp=None):
        ...
        adv = x2.copy()
        anchor = self.detector.get_feat(x1.astype(np.uint8))[0]
...
def main():
    ...
    reference_img = cv2.imread(os.path.join(input_dir, img_list[0])).astype(np.uint8)
    for idx in range(1, len(img_list)):
        ...
        try:
            target_img = cv2.imread(os.path.join(input_dir, img_list[idx])).astype(np.uint8)
