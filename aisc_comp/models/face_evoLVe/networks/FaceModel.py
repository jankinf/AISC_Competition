...
def get_model(url, net, embedding_size, no_gpu=False):
	...
	try:
		...
		checkpoint = torch.load('/data/public/models/face_recognition{}'.format(model_name),
				map_location=lambda storage, loc: storage.cpu())
	except Exception:
		print('No existing checkpoint, now downloading online')
		if not os.path.exists('/data/public/models/face_recognition'):
			try:
				os.makedirs('/data/public/models/face_recognition')
			except OSError as e:
				if e.errno != errno.EEXIST:
					raise
		urllib.request.urlretrieve(
			url,
			'/data/public/models/face_recognition{}'.format(model_name))
