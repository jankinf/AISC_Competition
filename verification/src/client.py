...
def read_image(image_path):
    ...
    assert os.path.exists(
        image_path[:-4] + '_BB.txt'), 'path not exists' + ' ' + image_path
...
def get_image(max_number=None):
    ...
    with open(GLOBAL_IMAGE_LIST_PATH) as f:
        image_list = json.load(f)
    logging.info("got local image list, {} image".format(
        len(image_list.keys())))
    Batch_size = 1024
