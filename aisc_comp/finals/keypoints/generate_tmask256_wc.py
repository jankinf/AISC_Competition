...
def get_bbox(kpoints, keyname, margin=0):
    ...
    if margin > 0 and keyname == 'EYE':
        x1 = min(256, round(x1) + margin)
    
    if margin > 0 and keyname == 'NOSE':
        y0 = max(0, round(y0) - margin)
        y1 = min(256, round(y1) + margin)

    if margin < 0 and keyname == 'NOSE':
        y0 = max(0, round(y0) + 1)
        y1 = min(256, round(y1) - 1)
    # elif margin < 0 and keyname == 'NOSE':
    #     x1 = min(256, round(x1) + margin)
...
def run_imgs():
    # kpoint_file = "/data/projects/aisc_facecomp/finals/keypoints/kpoints256.pkl"
    kpoint_file = args.kpoint_file
    # out_dir = "/data/projects/aisc_facecomp/finals/keypoints/show_tmasks256"
    out_dir = args.out_dir
    # mask_dir = "/data/projects/aisc_facecomp/finals/keypoints/tmasks256"
    mask_dir = args.mask_dir
    os.makedirs(out_dir, exist_ok=True)
    os.makedirs(mask_dir, exist_ok=True)
    # img_dir = "/data/projects/aisc_facecomp/finals/keypoints/face_aligned256"
    ...
    best_ratio = 0.11
    # with open("/data/projects/aisc_facecomp/finals/keypoints/generate_tmasks256.out", "a") as fp:
