...
def run_imgs():
    ...
    r = ScoreInference(model_path=args.i_net, device="cpu0", alpha=3, input_shape=(640, 480))
    ...
    # todo: load pattern
    pattern = np.load(args.pattern) # shape=(1, 3, 224, 224)
    pattern = ndimage.zoom(pattern, (1, 1, 640/224, 480/224), order=1)
    ...
    attack_dir = os.path.join(root_dir, args.name+"_pattern_rfix640", "attack")
    normal_dir = os.path.join(root_dir, args.name+"_pattern_rfix640", "normal")
    # attack_dir = os.path.join("/data/projects/fmp_demo/attack/adv_data/resize640x480", "attack")
    # normal_dir = os.path.join("/data/projects/fmp_demo/attack/adv_data/resize640x480", "normal")
