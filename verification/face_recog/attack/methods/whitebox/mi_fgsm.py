...
def mi_fgsm(feat1, x, model, num_iter, eps, alpha, fp=None, momentum=1, init=True, use_gpu=True):
    if use_gpu:
        feat1 = feat1.cuda()
        x = x.cuda()
        model = model.cuda()
    ...
    if init:
        adv = x + eps * (torch.rand_like(x).to(x.device) - 0.5) * 2
    else:
        adv = x.clone()
    ...
    with open(logfile, 'a') as fp:
        ...
        for idx in range(1, len(img_list)):
            ...
            try:
                ...
                adv_x = mi_fgsm(feat1=feat1, x=target_img, model=model, fp=fp, **kwargs)
            ...
            adv_x = mi_fgsm(feat1=feat1, x=target_img, model=model, fp=fp, **kwargs)
            out_eps = (adv_x - target_img).abs().max()
            logdata = "id:{}, file:{}, eps_check: {}\n\n".format(idx, img_list[idx], out_eps)
            print(logdata, end='')
            fp.write(logdata)
            fp.flush()
