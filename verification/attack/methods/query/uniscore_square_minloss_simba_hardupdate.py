...
class UniscoreAttack():
    ...
    def train(self, img_list, y, resume=False):
        ...
        for i_iter in range(int(n_queries - 1), self.n_queries):
            ...
            _x_bestcurr = x_best.copy()
            _margin_mincurr = margin_min.copy()
            _loss_mincurr = loss_min.copy()
            ...
            x_new_pos = _x_bestcurr + new_deltas
            ...
            x_new_neg = _x_bestcurr - new_deltas
            ...
            if loss_pos.sum() < loss_neg.sum():
                _loss_min = loss_pos
                _margin_min = margin_pos
                _x_best = x_new_pos
            else:
                _loss_min = loss_neg
                _margin_min = margin_neg
                _x_best = x_new_neg

            if _loss_min.sum() < _loss_mincurr.sum():
                logdata = "better >>> nquery:{}, loss_new:{}, loss_cur:{}, suc_new:{}, suc_cur:{}\n".format(
                    n_queries, _loss_min.sum().item(), _loss_mincurr.sum().item(), (_margin_min < 0.0).sum().item(), (_margin_mincurr < 0.0).sum().item())
