...
class UniscoreAttack():
    ...
    def train(self, img_list, y, resume=False):
        ...
        for i_iter in range(int(n_queries - 1), self.n_queries):
            ...
            # todo: search
            _new_deltas = self.eps * self.random_choice([n, c, h, w])
            _x_new = x_best_curr + _new_deltas
            _x_new = np.minimum(np.maximum(_x_new, img_batch - self.eps), img_batch + self.eps)
            _x_new = np.clip(_x_new, 0., 255.)

            _margin, _loss = self.margin_and_loss(_x_new, y)

            # update loss if new loss is better
            _idx_improved = (_loss < loss_min_curr).astype(np.float32)

            # todo: merge noise
            merge_noise = np.sum(_new_deltas * (_idx_improved * 2 - 1).reshape(-1, 1, 1, 1), axis=0, keepdims=True)
            momen_noise = grads + merge_noise / np.linalg.norm(merge_noise.reshape(-1), ord=1)
            new_deltas = self.eps * np.sign(momen_noise)
            ...
            if loss.sum() < loss_min_curr.sum() and (margin < 0.0).sum() >= (margin_min_curr < 0.0).sum():
                logdata = "better >>> nquery:{}, loss_search:{}, loss_new:{}, loss_cur:{}, suc_search:{}, suc_new:{}, suc_cur:{}\n".format(
                    n_queries, _loss.sum().item(), loss.sum().item(), loss_min_curr.sum().item(), (_margin < 0.0).sum().item(), (margin < 0.0).sum().item(), (margin_min_curr < 0.0).sum().item())
                ...
                logdata = "not better >>> nquery:{}, loss_search:{}, loss_new:{}, loss_cur:{}, suc_search:{}, suc_new:{}, suc_cur:{}\n".format(
                    n_queries, _loss.sum().item(), loss.sum().item(), loss_min_curr.sum().item(), (_margin < 0.0).sum().item(), (margin < 0.0).sum().item(), (margin_min_curr < 0.0).sum().item())
