...
def get_bbox(kpoints, keyname, margin=0):
    ...
    y1 = min(112, round(y1) + margin)
    x1 = min(112, round(x1) + margin)
    return [x0, y0, x1, y1]

def run_imgs():
    
    kpoint_file = "/data/projects/aisc_facecomp/finals/keypoints/kpoints.pkl"
    out_dir = "/data/projects/aisc_facecomp/finals/keypoints/show_tmasks"
    mask_dir = "/data/projects/aisc_facecomp/finals/keypoints/tmasks"
    ...
    best_ratio = 0.13
    with open("/data/projects/aisc_facecomp/finals/keypoints/generate_tmasks.out", "a") as fp:
