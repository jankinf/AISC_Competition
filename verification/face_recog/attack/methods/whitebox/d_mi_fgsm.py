...
def d_mi_fgsm(feat1, x, model, num_iter, eps, alpha, fp=None, momentum=1, init=True, use_gpu=True):
    if use_gpu:
        feat1 = feat1.cuda()
        x = x.cuda()
        model = model.cuda()
    ...
    with open(logfile, 'a') as fp:
        ...
        for idx in range(1, len(img_list)):
            ...
            try:
                ...
                adv_x = d_mi_fgsm(feat1=feat1, x=target_img, model=model, fp=fp, **kwargs)
