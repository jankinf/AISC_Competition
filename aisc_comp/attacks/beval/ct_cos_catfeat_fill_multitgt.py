...
def ct_cos_catfeat_fill_multitgt(images, 
        compare_images,
        keypoints_images,
        gkern,
        models_w, 
    ...
    images = (1 - mask) * images + mask * compare_images[0].unsqueeze(0)
    ...
    compare_logits_w = torch.concat(compare_feat_w, dim=0).reshape(1, n_model_w, comp_bs, -1) # (1, n_model_w, comp_bs, 512)
    compare_logits_b = torch.concat(compare_feat_b, dim=0).reshape(1, n_model_b, comp_bs, -1) # (1, n_model_b, comp_bs, 512)
